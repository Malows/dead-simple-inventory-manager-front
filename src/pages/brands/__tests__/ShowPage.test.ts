...
describe('brands/ShowPage.vue', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(store.deleteBrand).mockResolvedValue({ isOk: true } as never)
  })

  it('fetches the brand on mount and renders its details', async () => {
    ...
    expect(wrapper.text()).toContain(mockBrand.name)
    expect(wrapper.findComponent({ name: 'ProductList' }).props('products')).toEqual(mockBrand.products)
  })
  ...
  it('passes a delete action bound to the selected brand', async () => {
    const wrapper = mountComponent()
    await flushPromises()
    ...
    expect(deleteDialog.props('confirmMessage')).toBe(`brands.confirm_delete:${mockBrand.name}`)
    expect(store.deleteBrand).toHaveBeenCalledWith(mockBrand)
  })
})
