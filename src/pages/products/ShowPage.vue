...
const editRoute = computed(() => ({ name: 'products edit', params: route.params }))
const price = computed(() => product.value?.price ? parsePrice(product.value.price) : '')
</script>

<template>
  <page-with-actions v-if="product" title="Ver producto">
    <template #actions>
    ...
    <inline-data label="Nombre">{{ product.name }}</inline-data>
    ...
    <div class="input-row--md">
      ...
      <inline-data label="Advertencia de stock bajo">{{ product.min_stock_warning }}</inline-data>
    </div>

    <inline-data v-if="product.supplier" label="Proveedor">
      <router-link :to="{ name: 'suppliers show', params: { supplierId: product.supplier.uuid } }">
