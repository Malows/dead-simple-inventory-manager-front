...
describe('categories/ShowPage.vue', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(store.deleteCategory).mockResolvedValue({ isOk: true } as never)
  })

  it('fetches the category on mount and renders its details', async () => {
    ...
    expect(wrapper.findComponent({ name: 'ProductList' }).props('products')).toEqual(mockCategory.products)
  })
  ...
  it('passes a delete action bound to the selected category', async () => {
    const wrapper = mountComponent()
    await flushPromises()
    ...
    expect(deleteDialog.props('confirmMessage')).toBe(`categories.confirm_delete:${mockCategory.name}`)
    expect(store.deleteCategory).toHaveBeenCalledWith(mockCategory)
  })
})
